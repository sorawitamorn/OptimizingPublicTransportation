...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "city.chicago.cta.stations.table.v1":
            try:
                logger.info(f"process line info {message.topic()}")
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic() == "city.chicago.cta.station.arrivals.v2":
